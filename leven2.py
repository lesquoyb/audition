...
def Levenshtein(st1, st2, hmm):
    ...
    for i in range(0,len(st1)):
        for j in range(0,len(st2)):
            ...
            pomi = pomi if pomi != 0 else INC_MIN
            del_c = - log(pomi)

            pins = pins if pins != 0 else INC_MIN
            proba = proba_ins[st1[i-1]] if i > 0 and proba_ins[st1[i-1]] != 0 else INC_MIN
            ins_c = - log(pins) - log(proba)

            psub = psub if psub != 0 else INC_MIN
            proba = proba_sub[(l1, l2)] if l1 != "" and l2 != "" and proba_sub[(l1,l2)] != 0 else INC_MIN
            sub_c = 0   if l1 == l2 \
                        else - log(psub) - log(proba)
